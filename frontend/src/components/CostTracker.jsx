    ...
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        <div className="bg-white p-3 sm:p-4 rounded-lg border">
          <h4 className="text-xs sm:text-sm font-medium text-gray-500">Estimated Cost</h4>
          <p className="text-lg sm:text-xl md:text-2xl font-semibold text-blue-600">{formatCurrency(totals.estimates)}</p>
        </div>
        <div className="bg-white p-3 sm:p-4 rounded-lg border">
          <h4 className="text-xs sm:text-sm font-medium text-gray-500">Actual Expenses</h4>
          <p className="text-lg sm:text-xl md:text-2xl font-semibold text-orange-600">{formatCurrency(totals.expenses)}</p>
        </div>
        <div className="bg-white p-3 sm:p-4 rounded-lg border">
          <h4 className="text-xs sm:text-sm font-medium text-gray-500">Adjustments</h4>
          <p className="text-lg sm:text-xl md:text-2xl font-semibold text-purple-600">{formatCurrency(totals.adjustments)}</p>
        </div>
        <div className="bg-white p-3 sm:p-4 rounded-lg border">
          <h4 className="text-xs sm:text-sm font-medium text-gray-500">Net Balance</h4>
      ...
      {/* Add New Cost Form */}
      <form onSubmit={handleSubmit} className="bg-white p-4 sm:p-6 rounded-lg border space-y-4">
        <h3 className="text-base sm:text-lg font-semibold text-gray-800 mb-4">Add Cost Line</h3>
        ...
        <button
          type="submit"
          disabled={submitting}
          className="w-full sm:w-auto px-4 sm:px-6 py-2 text-sm sm:text-base bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed"
        >
              ...
              {costs.length === 0 ? (
                ...
                costs.map((cost) => (
                  <tr key={cost._id} className="hover:bg-gray-50">
                    <td className="px-3 sm:px-4 md:px-6 py-3 sm:py-4 text-sm text-gray-800">
                      <div>
                        ...
                        <div className="text-xs text-gray-500 mt-1 sm:hidden capitalize">
                          {cost.category || 'N/A'}
                        </div>
                      </div>
                    </td>
                    <td className="px-3 sm:px-4 md:px-6 py-3 sm:py-4 whitespace-nowrap text-sm text-gray-600 capitalize hidden sm:table-cell">
                      {cost.category || 'N/A'}
                    </td>
                    <td className="px-3 sm:px-4 md:px-6 py-3 sm:py-4 whitespace-nowrap text-xs sm:text-sm font-medium text-gray-800">
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto p-4 sm:p-6">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-base sm:text-lg font-semibold text-gray-800">Edit Cost Line</h3>
              <button
            ...
            <form onSubmit={handleEdit} className="space-y-4">
              ...
              <div className="flex flex-col sm:flex-row justify-end gap-3 pt-4">
                <button
                  type="button"
                  onClick={() => setEditingCost(null)}
                  className="px-4 py-2 text-sm border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 order-2 sm:order-1"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  disabled={submitting}
                  className="px-4 py-2 text-sm bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50 order-1 sm:order-2"
                >
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg max-w-md w-full p-4 sm:p-6">
            <h3 className="text-base sm:text-lg font-semibold text-gray-800 mb-2">Delete Cost Line</h3>
            ...
            <div className="flex flex-col sm:flex-row justify-end gap-3">
              ...
              <button
                ...
                className="px-4 py-2 text-sm bg-red-600 text-white rounded-lg hover:bg-red-700 disabled:opacity-50 order-1 sm:order-2"
                disabled={submitting}
              >
